...
static uint8_t readDS1307(uint8_t address, uint8_t *buf, uint8_t count) {
  // Send address of data.
  if (!i2c.transfer((DS1307_ADDRESS<<1) | I2C_WRITE, &address, 1, I2C_STOP)) return false;

  // Read data.
  return i2c.transfer((DS1307_ADDRESS<<1) | I2C_READ, buf, count, I2C_STOP);
}

//static uint8_t writeDS1307(uint8_t address, uint8_t *buf, uint8_t count) {
//  // Write address and continue transfer for write of data.
//  if (!i2c.transfer((DS1307_ADDRESS<<1) | I2C_WRITE, &address, 1, I2C_CONTINUE)) {
//    return false;
//  }
//  // Write data.
//  return i2c.transferContinue(buf, count);
//}
...
void RTC_DS1307::adjust(const DateTime& dt) {
  ...
  i2c.transfer((DS1307_ADDRESS<<1) | I2C_WRITE, buf, 9, I2C_CONTINUE);
}
